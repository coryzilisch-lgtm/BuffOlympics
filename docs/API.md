...
| Method & path | Body / returns |
|---|---|
| `GET /api/admin/overview` | See shape below. `dip.entries[]` carries `no` (the assigned ballot number, migration 014), `userId`, and `numbered` (false = still falling back to entry order). `people[]` carries **`email`** (admin-only payload — the player bootstrap never sends addresses) for the Admin Center's per-tribe email list + export. `relay.roster[legId][tribe][]` entries are `{id, name}` (the player payload sends names only) so the Relay tab can remove a specific person, and `relay.legByUser` maps `userId -> legId` so the + Add search can flag who's already running a leg. Also carries `signupMax` `{buffalo, roadhouse}` — the code-level per-tribe day cap (`api/lib/bootstrap.js`), which the Admin Center's **Slot report** (Games tab) uses to compare open seats against what each tribe may still claim. |
...
| `POST /api/admin/people` | `{userId, action:'toggleAdmin'|'toggleRef'|'addGame'|'removeGame'|'fillSlot'|'unfillSlot'|'fillRelay'|'unfillRelay'|'resetPassword'|'removeUser', gameId?, slotId?, legId?, password?}`. addGame/removeGame manage that user's `bo_signups` rows (admin override: ignores caps/limits/mode; addGame drops them into the game's earliest slot). `fillSlot` puts the user in a **specific** slot (`slotId`) — admin override, ignores caps/overlap/mode — powering "Fill slot" in the Games editor and the time-slot picker when adding someone to a game from the People tab. `unfillSlot` pulls the user OUT of a specific slot (`slotId`) — the reverse of `fillSlot`; powers the slot roster chips in the Games editor. `fillRelay` puts the user on a relay leg (`legId`) and `unfillRelay` takes them off it — the relay counterpart of fillSlot/unfillSlot, powering **+ Add** / × in the Relay tab. `fillRelay` is an admin override in the same way (ignores the leg cap and event mode) but keeps the model's own rule that a person runs ONE leg, so adding someone already on another leg MOVES them; 409 if they have no tribe (the relay roster is per tribe), 404 if the leg is gone. `unfillRelay` is scoped by `legId` so a stale click can't clear a row that has since moved. `resetPassword` sets a new `password_hash` from `password` (min 4 chars) — admin-driven reset for anyone who forgets theirs (no email infra; admin tells them in person). `removeUser` deletes the user + their sign-ups/dip/relay/ref-assignment (keeps logged `bo_results` history) — used for clearing test/bogus accounts. Returns `{ok:true}`. |
...
| `POST /api/ac/dip-entries` | Dip Off roster + ballot numbering (migration 014, `bo_dip_entries.dip_no`). Routed as `dip-entries`, NOT `dip`, because `ac-dip` already owns `ac/dip/{entryId}`.<br>`{action:'add', userId}` — add a cook. **Admin override**: ignores the five-per-tribe cap and the event mode. 409 if they have no tribe (the Dip Off is per tribe) or are already entered; the new entry gets `MAX(dip_no)+1`. Works pre-014 (numbering just stays derived).<br>`{action:'setNumber', entryId, no}` — assign the number voters see. If another entry already shows that number the two **swap** (a ballot can't have duplicates); returns `{ok:true, swappedWith:<entryId|null>}`. `no` must be 1–999.<br>`{action:'renumber'}` — reassign 1..N in the order currently shown, closing gaps left by removals. Returns `{ok:true, count}`.<br>`setNumber`/`renumber` return 409 `"Dip numbers need migration 014"` pre-migration. |
| `POST /api/ac/award` | `{team:'buffalo'|'roadhouse', pts, reason}` — award points straight to a tribe for something outside a game (sportsmanship, a tiebreak, a penalty). Writes a normal `bo_results` row: `game_name='Admin award'`, `detail`=`reason`, `winner`=`team`, `pts_buffalo`/`pts_roadhouse` set from `pts`, `entered_by`=the admin. **`player_name` is NULL** so a tribe award never appears on the individual leaderboard. `pts` must be a non-zero integer with `abs(pts) <= 999` — **negative deducts**. `reason` is **required** (400 if blank) and capped at 280 chars. Returns `{ok:true, team, pts}`. The row edits/deletes like any other result. |
