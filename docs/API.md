...
| Method & path | Body / returns |
|---|---|
| `GET /api/admin/overview` | See shape below. Also carries `signupMax` `{buffalo, roadhouse}` — the code-level per-tribe day cap (`api/lib/bootstrap.js`), which the Admin Center's **Slot report** (Games tab) uses to compare open seats against what each tribe may still claim. |
